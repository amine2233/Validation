...
public struct Validator<T>: CustomStringConvertible, Sendable {
    /// Readable name explaining what this `Validator` does. Must be suitable for placing after `is` _and_ `is
    /// not.
    ...
    private let closure: @Sendable (T) throws -> Void
    ...
    public init(_ readable: String, _ closure: @escaping @Sendable (T) throws -> Void) {
